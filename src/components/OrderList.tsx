...
const OrderList: React.FC<OrderListProps> = ({ orders, role, onUpdateStatus }) => {
          ...
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead className="bg-gray-50 dark:bg-gray-700">
              <tr>
                ...
                {isAdmin && <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Shipping Details</th>}
            ...
            <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              {orders.map((order) => (
                <tr key={order.id} className="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                  ...
                  {isAdmin && (
                    <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-400 min-w-[200px]">
                      {order.shippingAddress ? (
                        <div className="flex flex-col space-y-0.5">
                          <span className="font-bold text-gray-900 dark:text-white">{order.shippingAddress.fullName}</span>
                          <span className="text-xs">{order.shippingAddress.street}</span>
                          <span className="text-xs">{order.shippingAddress.city}, {order.shippingAddress.state} {order.shippingAddress.zipCode}</span>
                          <span className="text-xs font-bold uppercase tracking-wide text-brand-600 dark:text-brand-400 mt-1">{order.shippingAddress.country}</span>
                        </div>
                      ) : (
                  ...
                  {isAdmin && (
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                       <select 
                         ...
                         className="border border-gray-300 dark:border-gray-600 rounded text-xs px-2 py-1 outline-none focus:ring-1 focus:ring-brand-500 bg-white dark:bg-gray-700 dark:text-white cursor-pointer"
                       >
