...
const App = () => {
  ...
  const handleUpdateCartQty = async (id, qty) => {
    if (!user) return;
    try {
  ...
  const handleToggleWishlist = async (sweetId) => {
    if (!user) {
  ...
  const handlePlaceOrder = async (address, payment) => {
    if (!user) return;
    setLoading(true);
    try {
  ...
  const handleSaveSweet = async (sweet) => {
    try {
      if (sweet.id) {
        await sweetService.updateSweet(sweet);
        addToast('success', 'Sweet updated successfully');
      } else {
        const { id, ...newSweetData } = sweet; 
        await sweetService.addSweet(newSweetData);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-20 items-center">
          ...
          <div className="flex items-center space-x-2 md:space-x-4">
            ...
            ${user ? html`
              ${user.role === UserRole.CUSTOMER && html`
                     <button 
      ...
      <div className="flex flex-col md:flex-row gap-4">
        ...
        <div className="w-full md:w-56">
            ...
            >
                ...
                ${Object.values(SweetCategory).map(c => html`<option key=${c} value=${c}>${c}</option>`)}
            </select>
        </div>
  ...
  return html`
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex flex-col font-sans transition-colors duration-300">
      ...
      <main className="flex-grow w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        ` : html`
            ${(currentView === 'LOGIN' || currentView === 'REGISTER') && html`
