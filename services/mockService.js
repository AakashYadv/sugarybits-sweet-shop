import { SweetCategory, UserRole, OrderStatus } from '../types.js';
...
class MockBackendService {
  ...
  async getSweets(filters) {
    ...
    let sweets = data ? JSON.parse(data) : [];
    ...
    if (filters) {
      sweets = sweets.filter(sweet => {
        ...
        if (filters.minPrice !== undefined && filters.minPrice !== '' && !isNaN(filters.minPrice)) matches = matches && sweet.price >= filters.minPrice;
        if (filters.maxPrice !== undefined && filters.maxPrice !== '' && !isNaN(filters.maxPrice)) matches = matches && sweet.price <= filters.maxPrice;
        return matches;
      });
    }
    return sweets;
  ...
  getCartKey(userId) { return `${STORAGE_KEYS.CARTS}_${userId}`; }
  ...
  async updateCartItemQuantity(userId, sweetId, quantity) {
  ...
  async toggleWishlist(userId, sweetId) {
  ...
  async createOrder(userId, address, cartItems) {
